...
export default function App() {
  ...
  function handleAuth(role: AuthState["role"], name: string, email?: string) {
    const state: AuthState = { role, name, email };
    setAuth(state);
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-300 ${
      darkMode ? "bg-[#0A0A0F] text-[#E8E8F0]" : "bg-[#F5F5FA] text-[#1A1A2E]"
    }`}>
      <Header
        role={auth.role}
        name={auth.name}
      ...
      <main>
        ...
        {auth.role === "platform" && (
          <PlatformDashboard dark={darkMode} name={auth.name} />
        )}
        {auth.role === "admin" && (
          <AdminDashboard
            snapshots={snapshots}
