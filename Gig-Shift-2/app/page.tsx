...
export default function App() {
  ...
  if (appState === "register_rider" && loginData) return (
    <RiderRegistration
      email={loginData.email} mobile={loginData.mobile} language={loginData.language}
  ...
  return (
    <div className={darkMode ? "dark" : ""}>
      <div className="min-h-screen bg-white dark:bg-[#0C0C0C] transition-colors">
        <Header
          role={auth.role} name={auth.name} language={auth.language}
          darkMode={darkMode} toggleDark={() => setDarkMode(d => !d)}
          lastTick={lastTick} tickCount={tickCount} onLogout={handleLogout}
        />
        <main>
          {auth.role === "rider" && (
            <RiderDashboard snapshots={snapshots} weekly={weekly} pulseId={pulseId} dark={darkMode} name={auth.name} language={auth.language} />
          )}
          {auth.role === "platform" && (
